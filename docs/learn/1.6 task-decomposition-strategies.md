...
Task decomposition is how you break complex work into manageable pieces for an agentic system. The exam tests two distinct patterns and expects you to select the right one based on the characteristics of the task. It also tests a specific failure mode — attention dilution — that occurs when decomposition is insufficient.
...
> **Exam Trap — Suggesting a more powerful model or larger context window as the fix for attention dilution**
>
...
> **Exam Trap — Proposing a single-pass review with better prompts as equivalent to multi-pass architecture**
>
...
> **Exam Trap — Applying fixed pipelines to open-ended investigation tasks**
>
...
> **Exam Trap — Batching files into groups without adding a cross-file integration pass**
>
...
**Option A** — The model context window is too small to hold all 14 files — upgrade to a model with a larger context window

**Option B** — Split the review into per-file local analysis passes plus a separate cross-file integration pass to avoid attention dilution

**Option C** — Add a stronger system prompt emphasising the importance of reviewing all files with equal thoroughness

**Option D** — Reduce the number of files per review to 5 and process in sequential batches of 5 files each
